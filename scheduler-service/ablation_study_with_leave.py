...
def create_base_data():
    ...
    try:
        ...
        for emp in employees:
            if "offset" in emp:
                del emp["offset"]
        sys.stderr.write(f"Ablation: Loaded {len(employees)} employees from real_data.json\n")
    except Exception as e:
        sys.stderr.write(f"Ablation: Failed to load real_data.json: {e}.\n")
        sys.exit(1)
    ...
    east_reqs = {
        ...
        "GMDSS": 1, "Vista DO/ Sensitive Vessels": 1, "STW (TU)": 1, "Watch IC Console": 1 
    }
    west_reqs = {
        "East Control": 2, "West Control": 1, "Keppel": 1, "Sembawang Control": 1,
        "Sembawang MTC": 1, "STW (PB)": 1, "STW (TU)": 1, "Changi DO": 1
    }
...
def add_random_leave(data, percent):
    employees = data["employees"]
    all_dates = data["all_dates"]
    num_on_leave = int(len(employees) * (percent / 100))
    
    random.seed(42) # For reproducibility
    on_leave_pool = random.sample(employees, num_on_leave)
    
    leave_data = {}
    for emp in on_leave_pool:
        emp_id = emp["id"]
        # On leave for the entire month
        leave_data[emp_id] = {d: True for d in all_dates}
    
    data["leaveData"] = leave_data
    return data
...
def run_study():
    ...
    scenarios = [
        {"name": "0% Leave", "percent": 0},
        {"name": "10% Leave", "percent": 10},
        {"name": "15% Leave", "percent": 15},
    ]

    configs = [
        {"name": "Full Model", "pattern_weight": 5000, "understaff_weight": 10000000, "use_strategy": True},
    ...
    for scenario in scenarios:
        for config in configs:
            test_name = f"{config['name']} ({scenario['name']})"
            sys.stderr.write(f"Running: {test_name}...\n")
            
            data = add_random_leave(base_data.copy(), scenario['percent'])
            data["return_offsets"] = True
            
            scheduler4.PATTERN_PENALTY_WEIGHT = config['pattern_weight']
            scheduler4.UNDERSTAFFING_PENALTY_WEIGHT = config['understaff_weight']
            set_search_strategy(config['use_strategy'])
            
            start = time.time()
            output_json = scheduler4.main(data) 
            end = time.time()
            ...
            if "error" in result_data:
                ...
                penalty = result_data.get("objective", "N/A")
                solve_time = round(end - start, 2)
                unfilled = count_unfilled_slots(data, roster) 
                violations = count_pattern_violations(data, roster, offsets)
    ...
    print("\n--- Comprehensive Ablation Study with Leave Results ---")
